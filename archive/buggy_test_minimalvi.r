...
for (i in 1:nbsimul) {
	...
	Q<-QfromDelta(Delta,dist_mat,AS);
	...
	## Delta and x together
	# DeltaQx<-sampleDeltaQx(Delta,sdSampleDelta,y.r,x,K,Q,dist_mat,AS,muDelta,sdDelta);
	# Delta<-DeltaQx[[1]];
	# Q<-DeltaQx[[2]];
	# LLH<-DeltaQx[[3]];
	# x <- DeltaQx[[4]];
	...
	# ## Delta and x separetly
	# DeltaQ<-sampleDeltaQ(Delta,sdSampleDelta,x[1:dimension],K[1],Q,dist_mat,AS,muDelta,sdDelta);
	# Delta<-DeltaQ[[1]];
	# Q<-DeltaQ[[2]];
	# LLH<-DeltaQ[[3]];
	# x <- samplex(dimension,Q,K,y.r);
	# ##

	K <- sampleK(dimension,x,Kushape,Kuscale,Kvshape,Kvscale);
	K<-c(K,Delta,LLH);
	stop <- as.numeric(proc.time()[3]);
	cat("Ku",K[1],"Kv",K[2],"med x",median(x[1:dimension]),"[",max(x[1:dimension]),",",min(x[1:dimension]),"] \n");
	xsign<-(x[1:dimension])
	# titre<-paste("u");
	# plot_reel(data$easting,data$northing,xsign,main=titre)
	xsign<-(x[(dimension+1):(2*dimension)])
	titre<-paste("w");
	plot_reel(data$easting,data$northing,xsign,main=titre)

	# # dev.set(dev.prev())
	# mean_per_locality[5]<-aggregate(x[1:dimension],list(data$locality),mean)[2]
	# mean_per_locality[6]<-aggregate(x[(dimension+1):(2*dimension)],list(data$locality),mean)[2]
	# # plot(c(min(mean_per_locality$mean_data),max(mean_per_locality$mean_data)),c(-1.5,0),type="n")
	# # plot_text(mean_per_locality$mean_data,mean_per_locality$u,mean_per_locality$num)
	# # dev.set(dev.next())
	Ksamples[j,] <- K;
	if((i)%%freqsave==0 || i==(nbsimul)){
		write.table(t(x[1:dimension]), "usamples.txt", sep="\t",append=TRUE,col.names=FALSE,row.names=FALSE)
		# print(Ksamples[1:j,])
		write.table(Ksamples[1:j,], "Ksamples.txt", sep="\t",append=TRUE,col.names=FALSE,row.names=FALSE)
		Ksamples <- matrix(0,n,4);
		j=0;
	}
	...
	if((i)%%20==0){
		...
		if(rateaccept<0.3){
			sdSampleDelta<-0.9*sdSampleDelta;
			cat("update of sdSampleDelta to:",sdSampleDelta);
		}else if(rateaccept>0.7){
			sdSampleDelta<-1.1*sdSampleDelta;
			cat("update of sdSampleDelta to:",sdSampleDelta);
		}
	}
	scan()
...
# plot(data$easting,data$northing,pch=15,main=titre)
# lines(data$easting[y==TRUE],data$northing[y==TRUE],col=2,type="p",pch=15,cex=0.5)
# par(mfcol=c(2,5))
# for (i in 2:n) {
#   titre<-paste("u>0");
#   xsign<-(xsamples[1:dimension,i]>0)
# plot(data$easting,data$northing,pch=15,main=titre)
# lines(data$easting[xsign==TRUE],data$northing[xsign==TRUE],col=2,type="p",pch=15,cex=0.5)
# 
#   titre<-paste("x>0");
# xsign<-(xsamples[(dimension+1):(2*dimension),i]>0)
# # xsign<-(xsamples[1:dimension,i]>0)
# plot(data$easting,data$northing,pch=15,main=titre)
# lines(data$easting[xsign==TRUE],data$northing[xsign==TRUE],col=2,type="p",pch=15,cex=0.5)
# }

# mean_per_locality[5]<-aggregate(xsamples[1:dimension,n],list(data$locality),mean)[2]
# mean_per_locality[6]<-aggregate(xsamples[(dimension+1):(2*dimension),n],list(data$locality),mean)[2]
# names(mean_per_locality)<-c("num","mean_data","easting","northing","u","x")
