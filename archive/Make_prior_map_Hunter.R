##############
## Data Parameters (also see parameters_extrapol.r for computational parameters)
##############
# simulation name
nameSimul<-"extrapol_encMM_exp_f8.8T0.34_epsKc0.01StreetsTr50_insp0.891_nocof_nofitspat"

# map
db<-read.csv("DB_mm_blocks_05May2011.csv") #NB: carefull the unicodes are bullshit

# set columns for extrapol.spatautocorel
db$X<-db$easting
db$Y<-db$northing
db$opened<-as.numeric(db$status != 9)
db$infested<-as.numeric(db$status == 1)
db$block_num<-db$block_num
...
# computation of the extrapolation
updated<-extrapol.spatautocorel(db=db[,c("X","Y","opened","infested","block_num")])
save.image("make_prior_map_MM.img");
db$p.i<-updated$p.i
p.i<-db$p.i

##### checking
# general map of probability
full.screen()
par(mfrow=c(1,2))
plot_reel(db$X,db$Y,db$p.i,base=0,top=1,main="p.i")
plot_reel(db$X,db$Y,db$infested,base=0,top=1,main="data")
...
## table by locality
## total (missing) | raw estimate | Model estimate
byLoc<-aggregate(cbind(rep(1,dim(db)[1]),db[,c("opened","infested","p.i")]),by=list(db$locality),sum)
byLoc<-as.data.frame(byLoc)
names(byLoc)<-c("locality","total","open","obsInf","modelInf")

byLoc$rawEst<-byLoc$obsInf/byLoc$open
byLoc$modEst<-byLoc$modelInf/byLoc$total
byLoc$closed<-byLoc$total-byLoc$open


# byLoc<-byLoc[which(byLoc$rawEst>0),]

# add locality names
codigosLoc<-read.csv("CODIGOS_LOCALIDADES.csv")
MMNames<-codigosLoc[which(codigosLoc$P==1 & codigosLoc$D==10),]
indexInMMNames<-match(byLoc$locality,MMNames$L)
byLoc$names<-MMNames[indexInMMNames,"Localidad"]

# nice display
cat(file="byLocEst.csv",c("Locality","Total","Positive","Missing","Raw est.","Model est.\n"))
cat(file="byLocEst.csv",paste(byLoc$names,"\t",byLoc$total,"\t",byLoc$obsInf,"\t",byLoc$closed,"\t",round(byLoc$rawEst,4)*100,"\t",round(byLoc$modEst,4)*100,sep="",collapse="\n"),append=TRUE)
save.image("make_prior_map_MM.img");


# write.csv(db,file="roc_upto2009_p.i.csv",row.names=FALSE)

## estimate of the day after (specific to rociado data)
