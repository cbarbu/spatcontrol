# Works fine as long as we don't have an intercept on W
# the intercept is missing if not using the spatial component
graphics.off()
...
set.seed(777)
dbFit<-fit.spatautocorel(db=db,cofactors=c("CU","PE","oanimal","I.NO","P.NO"),nbiterations=-1,nocheck=TRUE,use.v=TRUE)
summary.spatcontrol(dbFit=dbFit)
