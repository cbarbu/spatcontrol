...
sdDprop <- 10 # standard deviation of the gaussian function to propose Delta in MH process
...
sample_Delta <- function(u,K,Delta,sdDprop,aD,bD,f,Q,LLHu,cholQ=NULL,Dmat=NULL){
	...
	llhDprop<-lik.Delta(Delta_prop,aD,bD);
	llhD<-lik.Delta(Delta,aD,bD);
	LLHproposal <- LLHuprop+llhDprop 
	LLH <- 	       LLHu+llhD;
	lnr <- LLHproposal-LLH+hasting_term;
	cat("Delta:",Delta,"LLH:",LLH,"(",LLHu,"+",llhD,")","Delta prop:",Delta_prop,"LLHproposal:",LLHproposal,"(",LLHuprop,"+",llhDprop,")","h_t",hasting_term,"lnr",lnr);
...
par(mfrow=c(1,3));
select<-c(zneg,zpos)
plot_reel(data$easting[select],data$northing[select],2*(data$status[select]-1),main=name)
scan()
...
## sampler
sampled<-as.matrix(mat.or.vec(nbsimul+1,9));
sampled[1,1]<-Delta;
sampled[1,2]<-0;
sampled[1,3]<-f;
sampled[1,4]<-0;
...
write.table(t(y), "ysamples.txt", sep="\t",col.names=FALSE,row.names=FALSE)
...
for (i in 1:nbsimul) {
	...
	if(i%%freqsave==0 || i==(nbsimul)){
		...
		write.table(t(y), "ysamples.txt", sep="\t",append=TRUE,col.names=FALSE,row.names=FALSE)
