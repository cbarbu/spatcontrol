...
## sampling functions
acceptDelta<-{};
sample_Delta <- function(u,K,Delta,sdDprop,aD,bD,f,cholQ=NULL){
	...
	LLHproposal <- lik.ugivQ(dimension,u,Qprop,K[1])+lik.Delta(Delta_prop,aD,bD); 
	LLH <- 	       lik.ugivQ(dimension,u,Q,K[1])+lik.Delta(Delta,aD,bD);
	...
	return(list(Delta,Q,LLH,cholQ));
}
library("stats")
acceptf<-{};
sample_f <- function(u,K,Delta,logsdfprop,f,mf,sdlf,cholQ=NULL){
	Q<-QfromDelta(Delta,dist_mat,AS,f=f);
	...
	LLHproposal <- lik.ugivQ(dimension,u,Qprop,K[1])+lik.f(f_prop,mf,sdlf); 
	LLH <- 	       lik.ugivQ(dimension,u,Q,K[1])+lik.f(f,mf,sdlf);
	lnr <- LLHproposal-LLH+hasting_term;
...
for (i in 1:nbsimul) {
	...
	out<-sample_f(u,K,Delta,logsdfprop,f,mf,sdlf); 
	f<-out[[1]];
	Q<-out[[2]];
	LLHf<-out[[3]];
	cholQ<-out[[4]];
	...
	out<-sample_Delta(u,K,Delta,sdDprop,aD,bD,f); 
	Delta<-out[[1]];
	Q<-out[[2]];
	LLHD<-out[[3]];
	cholQ<-out[[4]];
