...
## sampling functions
acceptDelta<-{};
sample_Delta <- function(u,y,zpos,zneg,bivect,K,Delta,sdDprop,aD,bD,f,Q,LLHu,cholQ=NULL,Dmat=NULL){
	...
	## sample u accordingly and get the corresponding likelihood of z
	cholQprop<-chol(Qprop);
	uprop<-sample_u(dimension,Qprop,K,y,cholQprop);
	LLHuprop<-lik.zgivw(uprop,zpos,zneg,bivect);
...
library("stats")
acceptf<-{};
sample_f <- function(u,y,zpos,zneg,bivect,K,Delta,logsdfprop,f,mf,sdlf,Q,LLHu,cholQ=NULL,Dmat=NULL){
	# Q<-QfromDelta(Delta,dist_mat,AS,f=f);
	...
	## sample u accordingly and get the corresponding likelihood of z
	uprop<-sample_u(dimension,Qprop,K,y,cholQ);
	LLHuprop<-lik.zgivw(uprop,zpos,zneg,bivect);
...
sampleKu <- function(dim,Q,u,Kushape,Kuscale) {
	pos.shape <- (0.5*(dim-1) + Kushape); # why dim *-1* removed for now
...
for (i in 1:nbsimul) {
	...
	cat("Ku:",Ku);
	uprop<-sample_u(dimension,Q,K,y,cholQ);
	LLHKuu<-lik.zgivw(uprop,zpos,zneg,bivect);

	out<-sample_Delta(u,y,zpos,zneg,bivect,K,Delta,sdDprop,aD,bD,f,Q,LLHKuu,Dmat=Dmat); 
	Delta<-out[[1]];
	Q<-out[[2]];
	LLHDu<-out[[3]];
	cholQ<-out[[4]];
	...
	out<-sample_f(u,y,zpos,zneg,bivect,K,Delta,logsdfprop,f,mf,sdlf,Q,LLHDu,cholQ=cholQ,Dmat=Dmat); 
	f<-out[[1]];
	Q<-out[[2]];
	LLHfu<-out[[3]];
	cholQ<-out[[4]];
...
dev.new()
smallsampled<-sampled[10000:200000,]
smallsampled<-smallsampled[seq(1,190000,10),]
...
best_LLH<-smallsampled[smallsampled$LLH>-340,]
dev.new()
par(mfrow=c(2,4))
hist(smallsampled)
dev.print(device=pdf,paste(simulname,"_general.pdf",sep=""))
dev.new()
par(mfrow=c(2,4))
hist(best_LLH)
dev.print(device=pdf,paste(simulname,"_bestLLHzgivu.pdf",sep=""))
