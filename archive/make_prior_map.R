...
nameSimul<-"extrapol_encMM_gau_epsKc0.01StreetsTr50_insp0.7_nocof_nofitspat_f15.1637"
## fn to set null to 0
...
# test<-set_to(roc,init=c("NULL"),final=0)
...
## full rociado map
roc<-read.csv("rociado_upto2009_programIyII_noaddicional.csv")
...
roc<-roc[which(!is.na(roc$X)),]

estimateunit<-rep(0,length(roc$X))
estimateunit[roc$region==1]<-1
estimateunit[roc$D==25]<-2
estimateunit[roc$D==7]<-3
estimateunit[roc$D==18 | roc$D==24]<-4
estimateunit[roc$D==23]<-5
estimateunit[roc$region==3]<-6
estimatedp.i<-rep(0,length(roc$X))

for(unit in 1:6){
	selG<-which(estimateunit==unit)

	source("extrapol_field.r")
	updated<-extrapol.spatautocorel(db=roc[selG,c("X","Y","opened","infested")])
	p.i<-updated$p.i
	estimatedp.i[selG]<-p.i

	#####
	full.screen()
	par(mfrow=c(1,2))
	plot_reel(roc$X[selG],roc$Y[selG],p.i,base=0,top=1,main="p.i")
	plot_reel(roc$X[selG],roc$Y[selG],roc$infested[selG],base=0,top=1,main="p.i")

	## checking
	db=roc[selG,c("X","Y","opened","infested")]
	full.screen()
	par(mfrow=c(1,3))
...
roc$est_p.i_db<-estimatedp.i # estimate of the day before
